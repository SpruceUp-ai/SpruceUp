...
| [spruceup/monitoring/monitor.py](spruceup/monitoring/monitor.py) | `Monitor`, `BaseWatcher`, `_BufferedQueue`, `_with_retry` — shared monitoring infrastructure |
| [spruceup/monitoring/local_file_watcher.py](spruceup/monitoring/local_file_watcher.py) | `LocalFileWatcher` — `_catch_up` + `_watch` for local filesystem sources |
